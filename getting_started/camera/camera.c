...
struct keys { int w; int a; int s; int d; };
...
void processInput(GLFWwindow *window, struct keys * keys){
	...
	int vals[] = {0,0,0,0};
	for (int i=0; i<4; i++){
		if (glfwGetKey(window, glfwKeys[i]) == GLFW_PRESS){
			vals[i] = 1;
		}
	}
	keys->w = vals[0];
...
int main(){
	...
	while (!glfwWindowShouldClose(window)){
		...
		vec3 perp;
		vec3_mul_cross(perp, cameraFront, cameraUp);
		vec3_norm( perp, perp );

		vec3_scale( wVec, cameraFront, ds*deltaTime );
		vec3_scale( aVec, perp, -ds*deltaTime );

		if (keys.w == 1) vec3_add( cameraPos, cameraPos, wVec );
		if (keys.a == 1) vec3_add( cameraPos, cameraPos, aVec );
		if (keys.s == 1) vec3_sub( cameraPos, cameraPos, wVec );
		if (keys.d == 1) vec3_sub( cameraPos, cameraPos, aVec );

		vec3 temp;
		vec3_add( temp, cameraPos, cameraFront );
		mat4x4_look_at( view, cameraPos, temp, cameraUp );

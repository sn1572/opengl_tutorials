...
char fragSource[] = "../../shaders/textures/fragEx4";
...
void processInput( GLFWwindow * window, GLint uniformLocation, float * alpha ){
	if ( glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS ){
		glfwSetWindowShouldClose(window, 1);
	}
	if ( glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS ){
		// increment alpha and reassign
		(*alpha) += 0.02;
		glUniform1f( uniformLocation, *alpha );
	}
	if( glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS ){
		// decrement alpha and reassign
		(*alpha) -= 0.02;
		glUniform1f( uniformLocation, *alpha );
	}
}
...
int main(){
	...
	// This call first gets the location of the variable texture2
	// from the (compiled first!) shader program. Then glUniform1i
	// assigned the value 3 to that variable. This will correspond
	// to which texture unit is being used.
	glUniform1i(glGetUniformLocation( (*shaders).ID, "texture2"), 3);

	// Good explanation here: 
	// https://www.opengl.org/sdk/docs/tutorials/ClockworkCoders/uniform.php
	GLint location = glGetUniformLocation( (*shaders).ID, "alpha" );
	float alpha = 0.2; // initial value for alpha
	...
	while (!glfwWindowShouldClose(window)){
		...
		processInput( window, location, &alpha );

...
char fragSource[] = "../../shaders/transformations/frag";
char vertSource[] = "../../shaders/transformations/vert";
...
int main(){
	...
	(*shaders).use( shaders );
	GLint loc = glGetUniformLocation( (*shaders).ID, "matrix" );
	...
	while (!glfwWindowShouldClose(window)){
		...
		mat4x4_rotate(M, M, 0, 0, 1, theta);
		mat4x4 T;
		mat4x4_translate(T, 0.5, -0.5, 0.0);
		mat4x4_mul(M, M, T);
		float flattened[16];
		flatten( flattened, M );
		...
		mat4x4 A;
		mat4x4_identity(A);
		float scale = (float)sin( (double)glfwGetTime() );
		//float scale = 0.5;
		for (int i=0; i<3; i++){
			A[i][i] = scale;
		}
		mat4x4 B;
		mat4x4_translate(B, -0.5, 0.5, 0.0);
		mat4x4_mul(A, B, A);
		flatten( flattened, A );
		if (first == 1){
			for (int i=0; i<16; i++){
